...
#     opt_D = optim.RMSprop(D.parameters(), lr=lr_D, alpha=0.999, eps=1e-08, weight_decay=0., momentum=0.999, centered=True)
    ...
    try:
        p_G = './saved_mask_landmarks_models/GEYE_latest.pth'
        p_D = './saved_mask_landmarks_models/DEYE_latest.pth'
...
#             L_rec7_diff = torch.sum(5. * torch.mean(torch.pow(torch.mul(torch.mul(Y,only_face_Xt),only_face_Y) - torch.mul(torch.mul(Xt,only_face_Xt),only_face_Y), 2).reshape(batch_size, -1), dim=1) * same_person.lt(1.)) / (same_person.lt(1.).sum() + 1e-6)
            ...
            lossG = 1.*L_adv + 10.*L_attr + 13.723*L_id + 8.*L_rec + L_landmarks + 1.*L_id_eye + L_id_eye_eye*1.
...
#             mask_reg = (face_mask_Y.bool() | face_mask_Xt.bool()).float()
            ...
            if iteration % show_step == 0:
                ...
                cv2.imwrite('./gen_images/latest_eye_AEI_landmarks_mask.jpg', (image*255).transpose([1,2,0]).astype(np.uint8))
            print(f'epoch: {epoch}    {iteration} / {len(dataloader)}')
            print(f'lossD: {lossD.item()}    lossG: {lossG.item()} batch_time: {batch_time}s')
            print(f'L_adv: {L_adv.item()} L_id: {L_id.item()} L_attr: {L_attr.item()} L_rec: {L_rec.item()}')
            
            if ((iteration % 100) == 0) and (iteration > 0):
                torch.save(G.state_dict(), './saved_mask_landmarks_models/GEYE_latest.pth')
                torch.save(D.state_dict(), './saved_mask_landmarks_models/DEYE_latest.pth')
        torch.save(G.state_dict(), './saved_mask_landmarks_models/GEYE_epoch_{}.pth'.format(epoch))
        torch.save(D.state_dict(), './saved_mask_landmarks_models/DEYE_epoch_{}.pth'.format(epoch))
